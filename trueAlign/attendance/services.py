...
class AttendanceAnalyticsService(BaseAttendanceService):
    ...
    def get_department_analytics(self, target_date: date) -> ServiceResult:
        ...
        try:
            ...
            # Get all users with their departments (assuming profile has department)
            departments = {}
            ...
            for user in users_with_dept:
                dept_name = getattr(user.profile, 'department', 'Unspecified') if hasattr(user, 'profile') else 'Unspecified'
    ...
    def get_top_performers(self, start_date: date, end_date: date, limit: int = 10) -> ServiceResult:
        ...
        try:
            ...
            for user in active_users:
                ...
                on_time_days = user_attendance.filter(status='Present').exclude(status__contains='Late').count()
    ...
    def get_attendance_concerns(self, start_date: date, end_date: date, limit: int = 10) -> ServiceResult:
        ...
        try:
            ...
            for user in active_users:
                ...
                total_days = user_attendance.count()
                if total_days < 5:  # Skip users with less than 5 days
                    continue
                ...
                # Flag as concern if absent > 20% or late > 30%
            ...
            # Sort by highest concern (absent_rate + late_rate)
    ...
    def get_monthly_summary(self, year: int, month: int) -> ServiceResult:
        ...
        try:
            ...
            summary = {
                ...
                'avg_hours': attendance_records.aggregate(Avg('total_hours'))['total_hours__avg'] or 0,
                'total_overtime': attendance_records.aggregate(Sum('overtime_hours'))['overtime_hours__sum'] or 0,
            }
            
            # Calculate percentages
            if summary['total_records'] > 0:
                summary['attendance_rate'] = round((summary['present_count'] / summary['total_records'] * 100), 1)
                summary['late_rate'] = round((summary['late_count'] / summary['total_records'] * 100), 1)
            else:
    ...
    def get_key_metrics(self, start_date: date, end_date: date) -> ServiceResult:
        ...
        try:
            ...
            total_records = attendance_records.count()
            present_count = attendance_records.filter(status__in=PRESENT_STATUSES).count()
            on_time_count = attendance_records.filter(status='Present').exclude(status__contains='Late').count()
            
            metrics = {
                'overall_attendance': round((present_count / total_records * 100) if total_records > 0 else 0, 1),
                'on_time_arrival': round((on_time_count / total_records * 100) if total_records > 0 else 0, 1),
                'average_hours': round(attendance_records.aggregate(Avg('total_hours'))['total_hours__avg'] or 0, 1),
                'total_overtime': round(attendance_records.aggregate(Sum('overtime_hours'))['overtime_hours__sum'] or 0, 1),
