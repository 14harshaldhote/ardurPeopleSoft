...
from .utils import (
    require_role, require_any_role, require_hr_or_admin,
...
def employee_dashboard(request):
    ...
    try:
        ...
        recent_requests = LeaveRequest.objects.filter(
            user=request.user
        ).select_related('leave_type', 'approver').order_by('-created_at')[:5]
...
def manager_dashboard(request):
    ...
    try:
        ...
        pending_approvals = LeaveRequest.objects.filter(
            ...
            status='Pending'
        ).select_related('user', 'leave_type').order_by('created_at')[:10]
...
def leave_balance(request):
    ...
    if is_hr(request.user) or is_admin(request.user):
        ...
        if user_id:
            try:
                user_to_view = User.objects.get(id=user_id, is_active=True)
