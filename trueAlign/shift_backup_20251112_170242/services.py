...
class ConflictDetector:
    ...
    def _check_leave_conflicts(self, user: User, check_date: date) -> List[ConflictDetail]:
        """Check for leave conflicts with proper leave management integration."""
        conflicts = []

        try:
            # Import leave models dynamically to avoid circular imports
            ...
            leave_requests = LeaveRequest.objects.filter(
                ...
                status__in=['approved', 'pending']  # Include pending for warnings
            ).select_related('leave_type')
            
            for leave_request in leave_requests:
                severity = "high" if leave_request.status == 'approved' else "medium"
                conflict_type = ConflictType.LEAVE_CONFLICT
                
                message = f"User is on {leave_request.status} {leave_request.leave_type.name if hasattr(leave_request, 'leave_type') else 'leave'} on {check_date}"
                suggestion = "Consider rescheduling the shift assignment" if leave_request.status == 'approved' else "Leave request is pending approval - monitor status"
            ...
            logger.debug("Leave management system not available - skipping leave conflict check")
        except Exception as e:
            logger.error(f"Error checking leave conflicts for user {user.id} on {check_date}: {e}")
            # Add a warning conflict for system error
            conflict = ConflictDetail(
                conflict_type=ConflictType.POLICY_VIOLATION,
                message=f"Unable to verify leave status - please check manually",
                severity="medium",
                suggestion="Verify user's leave status before finalizing assignment"
            )
            conflicts.append(conflict)
    ...
    def _clear_shift_caches(self):
        """
        Clear shift-related caches for better performance.
        """
        try:
            cache_patterns = [
                f"{CACHE_KEY_PREFIXES.get('shift_statistics', 'shift_stats_')}*",
                f"{CACHE_KEY_PREFIXES.get('user_current_shift', 'user_shift_')}*",
                f"{CACHE_KEY_PREFIXES.get('holidays', 'holidays_')}*",
            ]
            ...
            logger.error(f"Error clearing shift caches: {e}")
    
    def _clear_user_shift_cache(self, user_id: int):
        """
        Clear user-specific shift caches.
        ...
        try:
            cache_keys = [
                f"{CACHE_KEY_PREFIXES.get('user_current_shift', 'user_shift_')}{user_id}_*",
            ]
            ...
            logger.error(f"Error clearing user shift cache for user {user_id}: {e}")
...
class ShiftService:
    ...
    def get_shift_assignments(self, filters: Dict[str, Any] = None, page: int = 1, per_page: int = 50) -> Dict[str, Any]:
        ...
        Optimized to prevent N+1 queries and include comprehensive error handling.
        ...
        try:
            ...
            queryset = ShiftAssignment.objects.select_related(
                ...
                'user__groups',  # For role checking
                'shift__assignments'  # For conflict detection if needed
            )
            ...
            for assignment in paginated_queryset:
                try:
                    ...
                    user_full_name = self._get_user_display_name(assignment.user)
                    
                    # Get employee ID if available
                    employee_id = getattr(assignment.user, 'employee_id', None) or \
                                getattr(assignment.user, 'profile', {}).get('employee_id', None) if hasattr(assignment.user, 'profile') else None
                    ...
                    assignment_data = {
                        ...
                        'user': {
                            ...
                            'display_name': user_full_name,  # Consistent display name
                        },
                        'shift': {
                            ...
                            'crosses_midnight': assignment.shift.crosses_midnight,
                            'work_days': assignment.shift.work_days,
                        },
                        ...
                        'days_remaining': self._safe_call(assignment.days_remaining),
                        'total_duration': self._safe_call(assignment.total_duration),
                        'has_ended': self._safe_call(assignment.has_ended),
                        'status': self._get_assignment_status(assignment),
                    }
            ...
            logger.error(f"Error in get_shift_assignments: {e}")
            return {
                'assignments': [],
                'pagination': {
                    'current_page': page,
        ...
        try:
            ...
            for row_num, row in enumerate(csv_reader, start=2):  # Start at 2 (header is row 1)
                try:
                    ...
                    if not all([user_email, shift_name, effective_from_str]):
                        errors.append(f"Row {row_num}: Missing required fields (user_email, shift_name, effective_from)")
                        continue
                    ...
                    try:
                        ...
                        errors.append(f"Row {row_num}: User with email '{user_email}' not found")
                        continue
    ...
    def _get_user_display_name(self, user: User) -> str:
        """
        Get consistent user display name with proper fallback.
    ...
    def _safe_call(self, method_call):
        """
        Safely call a method and return None if it fails.
        
        Args:
            method_call: Method to call
    ...
    def _get_assignment_status(self, assignment) -> str:
        """
