...
import logging
import pytz
from ..models import ShiftAssignment, ShiftMaster, Holiday
...
# Type definitions for Django models to fix type checking
if TYPE_CHECKING:
    ...
    # Add type hints for Django model classes
    class ShiftMasterType:
        objects: Manager

    class ShiftAssignmentType:
        objects: Manager

    class HolidayType:
        objects: Manager

    ShiftMaster = ShiftMasterType
    ShiftAssignment = ShiftAssignmentType
    Holiday = HolidayType
...
class ShiftService:
    ...
    def get_recent_shift_changes(self, days: int = 7):
        """Get shift assignments that were recently created or ended within the last N days"""
        try:
            from ..models import ShiftAssignment
            ...
            # Get assignments recently created or recently ended
            recent_changes = ShiftAssignment.objects.filter(
                Q(effective_from__gte=start_date) |  # Recently started
                Q(effective_to__gte=start_date, effective_to__lte=today)  # Recently ended
            ).select_related('user', 'shift').order_by('-effective_from')

            return recent_changes
        except Exception as e:
            self.logger.error(f"Error fetching recent shift changes: {str(e)}")
            return []
