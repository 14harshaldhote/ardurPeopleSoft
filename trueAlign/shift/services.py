...
class ShiftService:
    ...
    def get_all_shifts(self, active_only: bool = True, page: int = 1, per_page: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            query = query.annotate(
                assignment_count=Count('shiftassignment'),
    ...
    def validate_shift_assignment(self, user_id: int, shift_id: int, effective_from: date,
                                effective_to: Optional[date] = None) -> Tuple[bool, str]:
        """
        Validate if a shift assignment is possible without conflicts.
        ...
        try:
            ...
            try:
                ...
                return False, "User or Shift not found"

            if not shift.is_active:
                return False, "Cannot assign inactive shift"
            ...
            overlapping_assignments = overlapping_query.exclude(
                effective_to__lt=effective_from
            )

            if overlapping_assignments.exists():
                assignment = overlapping_assignments.first()
                return False, f"Overlapping assignment exists: {assignment.shift.name} from {assignment.effective_from}"
