...
def bulk_assign_shift(request):
    ...
    if request.method == 'POST':
        ...
        assignment_method = request.POST.get('assignment_method', 'manual')
        
        if assignment_method == 'csv':
            # Handle CSV upload
            csv_file = request.FILES.get('csv_file')
            if csv_file:
                try:
                    result = shift_service.process_csv_bulk_assignment(csv_file, request.user)
                    if result['success']:
                        messages.success(request, f"CSV processed successfully! {result['assigned_count']} assignments created.")
            ...
            if form.is_valid():
                try:
                    ...
                    # Prepare assignment data for bulk processing
                    assignments_data = []
                    for user in users:
                        assignments_data.append({
                            ...
                            'notes': f'Bulk assignment by {request.user.get_full_name() or request.user.username}'
                        })
                    ...
                    if result.successful:
                        ...
                        messages.success(request,
                            f'Successfully assigned "{shift.name}" to {success_count} users.')

                    if result.failed:
                        failed_count = len(result.failed)
                        messages.error(request,
                            f'Failed to assign to {failed_count} users due to conflicts or errors.')
                    
                    if result.skipped:
                        skipped_count = len(result.skipped)
                        messages.warning(request,
                            f'Skipped {skipped_count} users due to conflicts.')
                    ...
                    error_msg = "Error processing bulk assignment. Please try again."
                    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
    ...
    try:
        ...
        shifts = ShiftMaster.objects.filter(is_active=True).annotate(
            active_assignments_count=Count('assignments', filter=Q(assignments__is_current=True))
        ).order_by('name')
        
        # Get active users (excluding those with current assignments for better UX)
        ...
        logger.error(f"Error getting bulk assign data: {e}")
        shifts = ShiftMaster.objects.none()
...
# ============================
# API ENDPOINTS FOR BULK ASSIGNMENT
# ============================
...
def api_validate_bulk_conflicts(request):
    ...
    try:
        ...
        except (ShiftMaster.DoesNotExist, User.DoesNotExist):
            return JsonResponse({'success': False, 'message': 'Invalid shift or users'})
            ...
            try:
                ...
                if existing_assignment:
                    conflicts.append({
                        ...
                        'message': f'Already assigned to {existing_assignment.shift.name}'
                    })
                else:
                    # Check for leave conflicts (if leave management is available)
                    try:
                        from trueAlign.leave_management.models import LeaveRequest
                        ...
                        if leave_conflicts:
                            warnings.append({
                                'user_id': user.id,
                                'user_name': user.get_full_name() or user.username,
                                'message': 'Has approved leave during this period'
                            })
                        else:
                            valid_assignments += 1
                    except ImportError:
                ...
                logger.error(f"Error checking conflicts for user {user.id}: {e}")
                warnings.append({
                    'user_id': user.id,
                    'user_name': user.get_full_name() or user.username,
                    'message': 'Could not validate - please check manually'
                })
        ...
        logger.error(f"Error in bulk conflict validation: {e}")
        return JsonResponse({'success': False, 'message': 'Validation failed'})

@login_required
def api_download_csv_template(request):
    """Download CSV template for bulk assignment"""
    try:
        # Create CSV content
        csv_content = [
            ['user_email', 'shift_name', 'effective_from', 'effective_to', 'notes'],
            ['user1@example.com', 'Morning Shift', '2024-01-01', '2024-12-31', 'Sample assignment'],
            ['user2@example.com', 'Evening Shift', '2024-01-01', '2024-12-31', 'Another assignment'],
            ['user3@example.com', 'Night Shift', '2024-01-01', '', 'Indefinite assignment']
        ]
        
        # Create HTTP response with CSV
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="bulk_assignment_template.csv"'
        ...
        return JsonResponse({'success': False, 'message': 'Failed to generate template'})

@login_required
@require_POST
def api_check_bulk_availability(request):
    """Check user availability for bulk assignment"""
    try:
        data = json.loads(request.body)
        shift_id = data.get('shift_id')
        date = data.get('date')
        ...
        if not all([shift_id, date, user_ids]):
            return JsonResponse({'success': False, 'message': 'Missing required data'})
        ...
        for user_id in user_ids:
            try:
                ...
                has_assignment = ShiftAssignment.objects.filter(
                    user=user,
                    is_current=True,
                ...
                conflicts = []
                if has_assignment:
                    conflicts.append('Already has shift assignment')
                
                # Check leave (if available)
                try:
                    from trueAlign.leave_management.models import LeaveRequest
                    ...
                    if has_leave:
                        conflicts.append('On approved leave')
                except ImportError:
                    pass
        ...
        logger.error(f"Error checking bulk availability: {e}")
        return JsonResponse({'success': False, 'message': 'Availability check failed'})

@login_required
def api_search_users(request):
    """Search and filter users for bulk assignment"""
    try:
        search_query = request.GET.get('q', '').strip()
        group_id = request.GET.get('group', '')
        availability_filter = request.GET.get('availability', '')  # 'available', 'assigned', 'all'
        ...
        if group_id:
            try:
                ...
                users_query = users_query.filter(groups=group)
            except Group.DoesNotExist:
                pass
        
        # Apply availability filter
        if availability_filter == 'available':
            # Exclude users with current assignments
            assigned_user_ids = ShiftAssignment.objects.filter(
                is_current=True,
                effective_from__lte=timezone.now().date()
            ).filter(
                Q(effective_to__isnull=True) | Q(effective_to__gte=timezone.now().date())
        ...
        elif availability_filter == 'assigned':
            # Only users with current assignments
            assigned_user_ids = ShiftAssignment.objects.filter(
                is_current=True,
                effective_from__lte=timezone.now().date()
            ).filter(
            ...
            users_query = users_query.filter(id__in=assigned_user_ids)
        
        # Get users with related data
        users = users_query.select_related().prefetch_related('groups')[:limit]
        ...
        for user in users:
            ...
            users_data.append({
                ...
                'current_shift': current_assignment.shift.name if current_assignment else None,
...
# CSV upload functionality is now handled by bulk_assign_shift view with assignment_method='csv'
