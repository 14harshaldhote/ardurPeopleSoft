...
window.SessionTracker = (function() {
    ...
    console.log('SessionTracker initialized with state:', state);

    // Initialize cross-tab communication
    const broadcastChannel = typeof BroadcastChannel !== 'undefined' ? 
        new BroadcastChannel('trueAlign_session_channel') : null;
...
function throttle(func, wait) {
        ...
        if (window.performance && window.performance.getEntriesByType) {
            ...
            if (resources && resources.length > 0) {
                metrics.resources = {
                    ...
                    avg_load_time: resources.reduce((sum, r) => sum + r.duration, 0) / resources.length
                };
            }
        }
    ...
    function checkExistingSession() {
        ...
        if (sessionParam) {
            ...
            console.log('Found session ID in URL parameters:', state.sessionId);
        }
        
        if (parentSessionParam) {
            state.parentSessionId = parentSessionParam;
            console.log('Found parent session ID in URL parameters:', state.parentSessionId);
        }
        
        // Check local storage
        if (!state.sessionId) {
            state.sessionId = localStorage.getItem(`${config.storagePrefix}sessionId`);
            console.log('Checked local storage for session ID:', state.sessionId);
        }
        
        if (!state.parentSessionId) {
            state.parentSessionId = localStorage.getItem(`${config.storagePrefix}parentSessionId`);
            console.log('Checked local storage for parent session ID:', state.parentSessionId);
        }
        ...
        if (storedTabId) {
            ...
            console.log('Found existing tab ID in session storage:', state.tabId);
        } else {
            sessionStorage.setItem(`${config.storagePrefix}tabId`, state.tabId);
            console.log('Generated new tab ID:', state.tabId);
        }
    }
...
function initializeSession() {
    ...
    console.log('Initializing session with client data:', clientData);
    ...
    if (state.sessionId) {
        ...
        sendApiRequest(config.apiEndpoints.updateSession, updateData, handleSessionResponse);
    } else {
        // Create new session
        const createData = {
            client_data: clientData
        ...
        sendApiRequest(config.apiEndpoints.createSession, createData, handleSessionResponse);
    }
    }
    ...
    function sendApiRequest(endpoint, data, callback) {
        ...
        .catch(error => {
            ...
            if (callback) callback({ success: false, error: 'Network error: ' + error.message });
        });
    }
    ...
    function handleSessionResponse(response) {
        if (response && response.success) {
            ...
            if (!state.parentSessionId && response.is_new_session) {
                ...
                localStorage.setItem(`${config.storagePrefix}parentSessionId`, state.parentSessionId);
                console.log('Set parent session ID:', state.parentSessionId);
            ...
            console.log('Session initialized with ID:', state.sessionId);
            ...
            if (response.is_new_session && broadcastChannel) {
                ...
                console.log('Broadcasting session info to other tabs:', broadcastMessage);
                broadcastChannel.postMessage(broadcastMessage);
                state.broadcast_messages_sent++;
            }
        } else {
            console.error('Failed to initialize session:', response ? response.error : 'Unknown error');
        }
    }
    ...
    function startTimers() {
        ...
        state.activitySampleTimer = setInterval(sampleActivity, config.activitySampleInterval);
        
        // Performance sampling timer
        state.performanceSampleTimer = setInterval(samplePerformance, config.performanceSampleInterval);
    }
    ...
    function updateConnectionInfo(connection) {
        state.deviceInfo.connectionType = connection.effectiveType || connection.type;
        sendApiRequest(config.apiEndpoints.updateSession, {
            session_id: state.sessionId,
            device_info: {
    ...
    function generateFingerprint() {
        ...
        console.log('Generated browser fingerprint:', state.browserFingerprint);
    }
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    console.log('Got location data:', state.locationData);
    ...
    function handleUserActivity() {
        ...
        if (wasIdle) {
            console.log('User activity detected, ending idle state');
            state.isIdle = false;
            ...
            if (state.idleStartTime) {
                ...
                console.log(`Added ${idleDuration}ms to total idle time, now: ${state.total_idle_time}ms`);
            }
            ...
            const idleStateChange = {
                action: 'idle_end',
                timestamp: new Date().toISOString(),
                duration: state.idleStartTime ? (now - state.idleStartTime) / 1000 : null // in seconds
            };
    ...
    function handleVisibilityChange() {
        ...
        console.log('Visibility changed:', visibilityData);
        ...
        if (state.sessionId) {
            ...
            console.log('Sending tab visibility update:', updateData);
    ...
    function handleWindowFocus() {
        ...
        console.log('Window focus gained:', focusData);
        ...
        if (state.backgroundStartTime) {
            ...
            console.log(`Added ${backgroundDuration}ms to background time, total: ${state.background_time}ms`);
        }
        ...
        if (state.sessionId) {
            const updateData = {
                ...
                is_active: true,
                tab_last_focus: new Date().toISOString(),
                background_time: state.background_time ? state.background_time / 1000 : 0 // Convert to seconds
            };
            
            console.log('Sending focus update:', updateData);
            sendApiRequest(config.apiEndpoints.updateSession, updateData);
        }
    ...
    function handleWindowBlur() {
        ...
        console.log('Window focus lost:', blurData);
        ...
        if (state.sessionId) {
            ...
            console.log('Sending blur update:', updateData);
            sendApiRequest(config.apiEndpoints.updateSession, updateData);
        }
    ...
    function trackKeyboard(event) {
        ...
        console.log('Keyboard event tracked:', keyboardData);
        ...
        if (state.activityBuffer.keystrokes.length >= 20) {
            console.log('Keyboard buffer full, triggering activity sample');
            sampleActivity();
        }
    }
    ...
    function trackScroll() {
        const scrollData = {
            ...
            scroll_max_y: Math.max(document.body.scrollHeight, document.documentElement.scrollHeight) - window.innerHeight,
        ...
        if (state.activityBuffer.scrolls.length >= 15) {
            console.log('Scroll buffer full, triggering activity sample');
            sampleActivity();
        }
    }
    ...
    function trackMouseMovement(event) {
        ...
        if (state.activityBuffer.mouseMoves.length % 10 === 0) {
            console.log(`Mouse movements tracked: ${state.activityBuffer.mouseMoves.length}`);
        }
        
        // If buffer is getting too large, send update
        if (state.activityBuffer.mouseMoves.length >= 50) {
            console.log('Mouse movement buffer full, triggering activity sample');
            sampleActivity();
        }
    }
    ...
    function heartbeat() {
        ...
        console.log(`Heartbeat check - Idle time: ${idleTime}ms, Threshold: ${config.idleThreshold}ms, Is idle: ${state.isIdle}`);
        ...
        if (state.isIdle && !state.idleWarningShown && 
        ...
        if (state.isIdle && (now - state.idleStartTime) >= config.autoLogoutThreshold) {
            console.log('Auto logout triggered after extended idle time');
            autoLogout();
            return;
        }
        ...
        if (state.sessionId) {
            ...
            const workingTime = Math.max(0, (sessionDuration * 1000 - totalIdleTime) / 1000);
            ...
            console.log('Sending heartbeat update with metrics:', heartbeatData);
    ...
    function sampleActivity() {
        console.log('Sampling activity data...');
        ...
        if (!state.sessionId || Object.values(state.activityBuffer).every(arr => arr.length === 0)) {
            return;
        }
    ...
    if (state.activityBuffer.keystrokes.length > 0) {
        ...
        console.log(`Sending ${state.activityBuffer.keystrokes.length} keyboard events`);
    ...
    if (state.activityBuffer.scrolls.length > 0) {
        ...
        console.log(`Sending ${state.activityBuffer.scrolls.length} scroll events`);
    ...
    if (state.activityBuffer.mouseMoves.length > 0) {
        ...
        state.mouse_movements = (state.mouse_movements || 0) + state.activityBuffer.mouseMoves.length;
        console.log(`Sending ${state.activityBuffer.mouseMoves.length} mouse movements, total: ${state.mouse_movements}`);
    ...
    if (state.activityBuffer.tabVisibility.length > 0) {
        ...
        console.log(`Sending ${state.activityBuffer.tabVisibility.length} tab visibility changes`);
        state.activityBuffer.tabVisibility = [];
    }
    
    // Add idle state changes if any
    if (state.activityBuffer.idleStateChanges && state.activityBuffer.idleStateChanges.length > 0) {
        activityData.idle_state_changes = state.activityBuffer.idleStateChanges;
        console.log(`Sending ${state.activityBuffer.idleStateChanges.length} idle state changes`);
        state.activityBuffer.idleStateChanges = [];
    }
    ...
    if (state.sessionStartTime) {
        ...
        if (state.isIdle && state.idleStartTime) {
            ...
            activityData.working_time = Math.max(0, activityData.session_duration - idleDuration);
        } else {
            activityData.working_time = activityData.session_duration;
        }
        
        console.log(`Session metrics - Duration: ${activityData.session_duration}s, Working time: ${activityData.working_time}s`);
    }
    ...
    sendApiRequest(config.apiEndpoints.updateSession, activityData, function(response) {
        if (response && response.success) {
            console.log('Activity data sent successfully');
        } else {
            console.error('Failed to send activity data:', response ? response.error : 'Unknown error');
        }
    });
    ...
    function showIdleWarning() {
        ...
        const remainingMinutes = Math.ceil((config.autoLogoutThreshold - (Date.now() - state.idleStartTime)) / 60000);
        ...
        document.getElementById('idle-warning-close').addEventListener('click', function(e) {
            e.stopPropagation();
            handleUserActivity();
            hideIdleWarning();
        });
    ...
    function autoLogout() {
        ...
        const now = Date.now();
        const sessionDuration = state.sessionStartTime ? (now - state.sessionStartTime) / 1000 : 0; // in seconds
        let totalIdleTime = 0;
        
        // Calculate total idle time
        if (state.isIdle && state.idleStartTime) {
            const currentIdleDuration = (now - state.idleStartTime) / 1000; // in seconds
            totalIdleTime = state.total_idle_time ? state.total_idle_time + currentIdleDuration : currentIdleDuration;
        ...
        console.log(`Final session metrics - Duration: ${sessionDuration}s, Total idle time: ${totalIdleTime}s, Working time: ${workingTime}s`);
        console.log(`Final scores - Productivity: ${state.productivity_score}, Engagement: ${state.engagement_score}, Quality: ${state.session_quality}`);
    ...
    return {
        ...
        getTabId: function() { return state.tabId; },
        getParentSessionId: function() { return state.parentSessionId; },
        isIdle: function() { return state.isIdle; },
        getIdleTime: function() {
            return state.isIdle ? Math.floor((Date.now() - state.idleStartTime) / 1000) : 0;
        },
        getRemainingTime: function() {
            if (!state.isIdle) return config.autoLogoutThreshold / 1000;
            const remaining = config.autoLogoutThreshold - (Date.now() - state.idleStartTime);
            return Math.max(0, Math.floor(remaining / 1000));
        },
        getDeviceInfo: function() { return { ...state.deviceInfo }; },
        getLocationData: function() { return state.locationData; },
        getVisitedUrls: function() { return { ...state.visitedUrls }; },
        getPerformanceMetrics: function() { return { ...state.performanceMetrics }; },
        endSession: function() {
            if (state.sessionId) {
                // Calculate final session metrics
                const now = Date.now();
                ...
                console.log(`Manual end session - Final metrics - Duration: ${sessionDuration}s, Total idle time: ${totalIdleTime}s, Working time: ${workingTime}s`);
                console.log(`Final scores - Productivity: ${state.productivity_score}, Engagement: ${state.engagement_score}, Quality: ${state.session_quality}`);
